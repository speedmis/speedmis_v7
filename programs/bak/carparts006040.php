...
function misMenuList_change() {
	...
	if($real_pid=='carparts006076') {
		$temp1 = $misSessionUserId;
		if($temp1=='admin' || $temp1=='gadmin') {
			$temp1 = '포천';
		}
		$result[0]['g09'] = " and table_m.g4name='$temp1'";
	}
...
.k-grid tbody .k-button {
	...
	body[realpid="carparts006076"] td[comment="Y"] > a[onclick] {
		display:none;
	}
</style>
...
*/
function move_storage(p_this, p_move) {
	idx = $(p_this).attr('idx');
...
function add_storage(p_this, p_add, p_depth) {
...
function open_info(p_this) {
	...
	idx = $(p_this).attr('idx');
	url = 'index.php?RealPid=carparts006078&idx='+idx+'&ActionFlag=modify';
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='g4name') {
		...
		rValue0 = `
<a onclick="add_storage(this,'down',1);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
...
`;
		...
		if(p_dataItem['autogubun']!='0000' && Len(p_dataItem['autogubun'])==4 ) {
			rValue1 = `
<a onclick="move_storage(this,'top');" idx="`+p_dataItem['idx']+`" class="k-button hide"`+iif(p_dataItem['autogubun']=='0001',' disabled','')+` title="맨위로 이동">↑</a>
<a onclick="move_storage(this,'up');" idx="`+p_dataItem['idx']+`" class="k-button"`+iif(p_dataItem['autogubun']=='0001',' disabled','')+` title="한칸 위로 이동">▲</a>
<a onclick="move_storage(this,'down');" idx="`+p_dataItem['idx']+`" class="k-button" title="한칸 아래로 이동">▼</a>
<a onclick="move_storage(this,'bottom');" idx="`+p_dataItem['idx']+`" class="k-button hide" title="맨 아래로 이동">↓</a>
`;
		}
		...
		if(p_dataItem['autogubun']!='0000' && Len(p_dataItem['autogubun'])>=4 && Len(p_dataItem['autogubun'])<=8) {
			rValue0 = `
<a onclick="add_storage(this,'down',2);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
`;
		...
		if(Len(p_dataItem['autogubun'])==8) {
			rValue1 = `
<a onclick="move_storage(this,'top');" idx="`+p_dataItem['idx']+`" class="k-button hide"`+iif(Right(p_dataItem['autogubun'],4)=='0001',' disabled','')+` title="맨위로 이동">↑</a>
<a onclick="move_storage(this,'up');" idx="`+p_dataItem['idx']+`" class="k-button"`+iif(Right(p_dataItem['autogubun'],4)=='0001',' disabled','')+` title="한칸 위로 이동">▲</a>
<a onclick="move_storage(this,'down');" idx="`+p_dataItem['idx']+`" class="k-button" title="한칸 아래로 이동">▼</a>
<a onclick="move_storage(this,'bottom');" idx="`+p_dataItem['idx']+`" class="k-button hide" title="맨 아래로 이동">↓</a>
`;
		}
		...
		if(Len(p_dataItem['autogubun'])>=8 && Len(p_dataItem['autogubun'])<=12) {
			rValue0 = `
<a onclick="add_storage(this,'down',3);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=12 && Len(p_dataItem['autogubun'])<=16) {
			rValue0 = `
<a onclick="add_storage(this,'down',4);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=16 && Len(p_dataItem['autogubun'])<=20) {
			rValue0 = `
<a onclick="add_storage(this,'down',5);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=20 && Len(p_dataItem['autogubun'])<=24) {
			rValue0 = `
<a onclick="add_storage(this,'down',6);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=24 && Len(p_dataItem['autogubun'])<=28) {
			rValue0 = `
<a onclick="add_storage(this,'down',7);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=28 && Len(p_dataItem['autogubun'])<=32) {
			rValue0 = `
<a onclick="add_storage(this,'down',8);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=32 && Len(p_dataItem['autogubun'])<=36) {
			rValue0 = `
<a onclick="add_storage(this,'down',9);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
`;
		...
		if(Len(p_dataItem['autogubun'])>=36 && Len(p_dataItem['autogubun'])<=40) {
			rValue0 = `
<a onclick="add_storage(this,'down',10);" idx="`+p_dataItem['idx']+`" class="k-button" title="바로 아래에 추가">+</a>
`;
		} else {
			rValue0 = `
<a class="k-button invisible"></a>
...
function list_json_init() {
    global $real_pid, $misJoinPid, $logicPid, $parent_idx, $full_siteID, $misSessionUserId;
    ...
    if($flag=='read' && $selField=='') { 
        if($real_pid=='carparts006076') {
            ...
            $appSql = "select count(*) from mis_parts_storage WHERE upidx=1 AND storage_name='$temp1' ";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==0) {
                $appSql = "
INSERT INTO mis_parts_storage (
    storage_name, 
    upidx, 
    fixgubun, 
    autogubun, 
...
SELECT 
    '$misSessionUserId' AS storage_name,
    upidx,                  -- 기존값 그대로
    formatnums(CAST(fixgubun AS UNSIGNED) + 1,'00'),
    formatnums(CAST(autogubun AS UNSIGNED) + 1,'0000'),
    sortG01, sortG02, sortG03, sortG04, sortG05, 
    sortG06, sortG07, sortG08, sortG09, sortG10, 
    depth, '$misSessionUserId'
FROM mis_parts_storage 
WHERE upidx = 1 
ORDER BY autogubun DESC 
LIMIT 1;
				";
				execSql($appSql);
...
function addLogic_treat() {
	...
	if($question=="move") {
		$sql = " select sortG01,sortG02,sortG03,sortG04,sortG05,sortG06,sortG07,sortG08,sortG09,sortG10,depth from mis_parts_storage where idx='$p_idx' ";
		$r = allreturnSql($sql);
		$depth = $r[0]['depth'];
		$sortG01 = $r[0]['sortG01'];
		$sortG02 = $r[0]['sortG02'];
		...
		$sql = " 
		update mis_parts_storage set sortG01='$sortG01',sortG02='$sortG02',sortG03='$sortG03',sortG04='$sortG04',sortG05='$sortG05' 
		,sortG06='$sortG06',sortG07='$sortG07',sortG08='$sortG08',sortG09='$sortG09',sortG10='$sortG10'
		where idx='$p_idx';
		call mis_parts_storage_Ordering_Proc();		-- 여기서 vv_mis_parts_storage_tree 로 생성시킴
		";
		//echo $sql;
		execSql($sql);
		echo 'success';
		...
		$sql = " select sortG01,sortG02,sortG03,sortG04,sortG05,sortG06,sortG07,sortG08,sortG09,sortG10
		,upidx,depth from mis_parts_storage where idx='$p_idx' ";
		$r = allreturnSql($sql);
		...
		$sql = " 
		insert into mis_parts_storage (storage_name, upidx, sortG01, sortG02, sortG03, sortG04, sortG05, sortG06, sortG07, sortG08, sortG09, sortG10
		, depth, wdater) 
		values (N'$add_name', $upidx, '$sortG01', '$sortG02', '$sortG03', '$sortG04', '$sortG05', '$sortG06', '$sortG07', '$sortG08', '$sortG09', '$sortG10'
