...
INSERT INTO mis_parts_assemble (assemble_id, car_parts_id)
SELECT
    $parent_idx AS assemble_id,
    cp.id AS car_parts_id  -- car_parts 테이블의 고유 ID를 가져옵니다.
FROM
    car_parts AS cp
WHERE
    -- 1. id=95 레코드의 part_cate_fullname 값을 찾아 동적으로 LIKE 검색합니다.
    cp.part_cate_fullname LIKE CONCAT(
        (SELECT part_cate_fullname FROM car_parts WHERE id = $parent_idx),
        '%'
    )
    -- 2. NOT EXISTS를 사용하여 중복 방지 로직을 명확하게 적용합니다.
    -- (mis_parts_assemble 테이블에 'assemble_id=$parent_idx'와 현재 부품 ID(cp.id) 조합이 없는 경우)
    AND NOT EXISTS (
        SELECT 1
        FROM mis_parts_assemble AS mpa
        -- mis_parts_assemble 테이블의 'assemble_id'가 $parent_idx 이고,
        WHERE mpa.assemble_id = $parent_idx
          -- mpa의 부품 ID (이전에 assemble_id/cate_id로 쓰던 필드와 매칭)가 cp.id와 일치하는지 확인합니다.
          -- (주의: 기존 쿼리의 mpa.car_parts_id 필드가 '부품 ID'를 저장한다고 가정합니다.)
          AND mpa.car_parts_id = cp.id
    );
