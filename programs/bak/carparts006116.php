...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='virtual_fieldQncancel_fixed') {

		var rValue = '';
		if(p_dataItem['read_only_condition']=='1') {
			rValue = '<a class="k-button" onclick="$(`input#isFixed`)[0].checked=false;$(`#btn_save`).click();">확정 취소</a>';
		} 
        return rValue;
    } else {
...
function list_json_init() {
    ...
    if($grid_load_once_event=='N') { 
		$appSql = "CALL sp_sync_mis_parts_assy_define();";		//항상 새롭게 갱신.
		execSql($appSql);
    }
