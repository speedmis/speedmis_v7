...
function pageLoad() {
	...
	if($sid!='') {
		...
		if($full_name!='') {
			$url = 'index.php?gubun=6052&allFilter=[{"operator":"contains","value":"' . $full_name . '","field":"table_storage_cate_idxQnfull_name"}]&isMenuIn=Y';
			re_direct($url);
		} else {
?>
<script>
	...
	location.href = "index.php?gubun=6052&isMenuIn=Y";
</script>
<?php
		}
		exit;
    } else if($pid!='') {
		$sql = "SELECT part_cate_fullname FROM car_parts  WHERE id=$pid;";
		$full_name = onlyOnereturnSql($sql);
		if($full_name!='') {
			$url = 'index.php?gubun=6052&allFilter=[{"operator":"contains","value":"' . $full_name . '","field":"table_part_cate_idxQnfull_name"}]&isMenuIn=Y';
			re_direct($url);
		} else {
?>
<script>
	alert('해당 부품번호: <?php echo $sid; ?> 는 존재하지 않습니다.');
	location.href = "index.php?gubun=6052&isMenuIn=Y";
</script>
<?php
		}
...
body[userid="admin"] .before_round_virtual_fieldQnprint_request
,body[userid="admin"] div#round_virtual_fieldQnprint_request
,body[userid="admin"] div#round_print_request_time
,body[userid="admin"] div#round_print_response_time
	
,body[userid="gadmin"] .before_round_virtual_fieldQnprint_request
,body[userid="gadmin"] div#round_virtual_fieldQnprint_request
,body[userid="gadmin"] div#round_print_request_time
,body[userid="gadmin"] div#round_print_response_time {
...
function save_updateQueryBefore() {
    ...
    global $resultCode, $resultMessage;

	//아래는 업데이트 쿼리에 특정쿼리를 더 추가합니다.
$sql = $sql . " 

UPDATE car_parts set updated_at=CURRENT_TIMESTAMP where id=$key_value;

 UPDATE car_parts AS cp
JOIN parts_cate_tree_c AS vc
  ON cp.part_cate_idx = vc.abidx
SET cp.part_cate_fullname = vc.full_name
WHERE cp.part_cate_idx IS NOT NULL AND cp.part_cate_idx = vc.abidx 
and cp.id=$key_value;
