...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='it_id') {


		var rValue = '<a href="index.php?RealPid=' + parent.document.getElementById('RealPid').value + '&idx='+p_dataItem[p_aliasName]+'&isMenuIn=Y" target="_blank" class="k-button">새창</a>';
		rValue = rValue + p_dataItem['it_id'];
        return rValue;
    } else {
...
function list_query() {
    ...
    if($flag=="read") { 
		...
		$countQuery = replace($countQuery, "count(*)","COUNT(DISTINCT table_m.it_id)");
		$countQuery = replace($countQuery, "v_g5_shop_item_part_numbers table_m","v_g5_shop_item_part_numbers table_m JOIN g5_shop_item_part_numbers p1 ON p1.part_number = table_m.part_number");
		$countQuery = replace($countQuery, "and table_m.it_name","AND table_m.it_stock_qty > 0 
AND (
    (SELECT part_union_level FROM g5_shop_item WHERE it_id = '$parent_idx') = '2'   -- Assy 
    AND table_m.part_union_level='2'
OR 
    (SELECT part_union_level FROM g5_shop_item WHERE it_id = '$parent_idx') <> '2' 
    AND table_m.part_union_level<>'2'
) and p1.it_id
");
...
SELECT 
    it_id,
    CASE 
        WHEN LAG(it_id) OVER (ORDER BY it_id, part_number) = it_id THEN NULL 
        ELSE it_name 
    END AS it_name,
    CASE 
        WHEN LAG(it_id) OVER (ORDER BY it_id, part_number) = it_id THEN NULL 
...
FROM (
    SELECT DISTINCT 
        table_m.it_id,
        table_m.it_name,
        case when table_m.part_union_level='2' then 'Y' else 'N' END AS zAssy,
        table_m.it_stock_qty,
        table_m.part_number,
        DATE_FORMAT(table_m.wdate, '%Y-%m-%d %H:%i:%s') AS wdate
    FROM v_g5_shop_item_part_numbers table_m 
    JOIN g5_shop_item_part_numbers p1 ON p1.part_number = table_m.part_number
    WHERE table_m.it_stock_qty > 0 
      AND p1.it_id = '1769773436'
      -- 기준 아이템의 Assy 포함 여부와 동일한 데이터만 필터링
      AND (
          (SELECT part_union_level FROM g5_shop_item WHERE it_id = '$parent_idx') = '2'   -- Assy 
          AND table_m.part_union_level='2'
      OR 
...
function list_json_load() {
    ...
    if($flag=='read' && $selField=='') { 
		...
		$now_it_id = '';
        for($i=0;$i<$count;$i++) {
			$p_data[$i]['rowNumber'] = $i+1;
			if($i>=1) {
				if($p_data[$i-1]['it_id']!='') {
					$now_it_id = $p_data[$i-1]['it_id'];
				}
				if($now_it_id==$p_data[$i]['it_id']) {
					$p_data[$i]['it_id'] = '';
				}
			}
		}
