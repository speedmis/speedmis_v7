...
function open_info(p_this) {
	...
	idx = $(p_this).attr('idx');
	url = 'index.php?RealPid=carparts006062&idx='+idx+'&ActionFlag=modify';
...
";
                if(execSql($appSql)) {
                    $resultCode = "success";
                    $resultMessage = "DB반영이 완료되었습니다. ";
                } else {
                    $resultCode = "fail";
                    $resultMessage = "DB반영 처리가 실패하였습니다.";
                }
...
function addLogic_treat() {
	...
	if($question=="move") {
		$sql = " select sortG01,sortG02,sortG03,depth from mis_parts_cate_a where idx='$p_idx' ";
		$r = allreturnSql($sql);
		$depth = $r[0]['depth'];
		$sortG01 = $r[0]['sortG01'];
		$sortG02 = $r[0]['sortG02'];
		...
		$sql = " 
		update mis_parts_cate_a set sortG01='$sortG01',sortG02='$sortG02',sortG03='$sortG03'
		where idx='$p_idx';
		call mis_parts_cate_a_Ordering_Proc();
		";
		//echo $sql;
		execSql($sql);
		echo 'success';
		...
		,upidx,depth from mis_parts_cate_a where idx='$p_idx' ";
		$r = allreturnSql($sql);
		...
		$sql = " 
		insert into mis_parts_cate_a (cate_name, upidx, sortG01, sortG02, sortG03
		, depth, wdater) 
