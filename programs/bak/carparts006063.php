...
function list_json_init() {
    ...
    if($flag=='read') { 
        if($app=='DB반영') {
            $appSql = "select count(idx) from mis_parts_cate_b where sortG01='0' and useflag=1 ";
            $cnt = onlyOnereturnSql($appSql);
            if($cnt==1) {
                if($MS_MJ_MY=='MY') {
                    $appSql = "call mis_parts_cate_b_Ordering_Proc();";
                } else {
                    $appSql = "EXECUTE mis_parts_cate_b_Ordering_Proc";
                }
                if(execSql($appSql)) {
                    $resultCode = "success";
...
//exit;
	...
	$autogubun = onlyOnereturnSql("select autogubun from mis_parts_cate_b where idx='" . $updateList["upidx"] . "'");
