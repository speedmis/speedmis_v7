...
function misMenuList_change() {
	...
	//기본필터의 예
	if($real_pid=='carparts006125') {
		$result[0]['g09'] = "and table_m.site_id='01_부자톡' and table_m.steps in ('상신','검수완료')";
	}
...
function columns_templete(p_dataItem, p_aliasName) {
    ...
    if(p_aliasName=='zbullyu') {
		var rValue =  p_dataItem[p_aliasName] + '<br><a  href="index.php?gubun=6083&idx=' 
				+ p_dataItem['it_id'] + '&isMenuIn=Y" target="_blank" class="k-button">gadmin</a>';
		rValue = rValue + '<a href="/shop/item.php?it_id=' 
				+ p_dataItem['it_id'] + '" target="_blank" class="k-button">shop</a>';
        return rValue;
    } else if(p_aliasName=='siQnit_img1') {
...
function list_json_init() {
    ...
    if($flag=='modify' || $flag=='view') { 
            $appSql = "

INSERT INTO g5_shop_item_compare (it_id, site_id, wdater, useflag)
SELECT it_id, site_id, 'gadmin', '1'
FROM (
    SELECT '$idx' AS it_id, '01_부자톡' AS site_id UNION ALL
    SELECT '$idx', '02_이베이' UNION ALL
    SELECT '$idx', '03_ok파츠' UNION ALL
    SELECT '$idx', '04_지파츠' UNION ALL
    SELECT '$idx', '05_번개장터' UNION ALL
    SELECT '$idx', '06_파츠핏' UNION ALL
    SELECT '$idx', '07_중고나라' UNION ALL
    SELECT '$idx', '08_쿠팡' UNION ALL
    SELECT '$idx', '09_네이버스토어' UNION ALL
    SELECT '$idx', '10_기타$' UNION ALL
    SELECT '$idx', '11_기타$' UNION ALL
    SELECT '$idx', '12_기타원' UNION ALL
    SELECT '$idx', '13_기타원' UNION ALL
    SELECT '$idx', '14_기타원' UNION ALL
    SELECT '$idx', '15_기타원' UNION ALL
    SELECT '$idx', '16_기타원' UNION ALL
    SELECT '$idx', '17_기타원' UNION ALL
    SELECT '$idx', '18_기타원' UNION ALL
    SELECT '$idx', '19_기타원' UNION ALL
    SELECT '$idx', '20_기타원'
) AS tmp
WHERE NOT EXISTS (
    SELECT 1 
