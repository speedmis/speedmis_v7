...
function thisLogic_toolbar() {
    ...
    $("a#btn_1").text("정렬");
    $("li#btn_1_overflow").text("정렬");
    $("#btn_1").css("background", "#88f");
    $("#btn_1").css("color", "#fff");
    $("#btn_1").click( function() {
        $("#grid").data("kendoGrid").dataSource.transport.options.read.data.app = "정렬";
...
function list_json_init() {
    ...
    if($grid_load_once_event=='N') { 
		$appSql = "CALL sp_manage_assy_detail('$parent_idx');";		//항상 새롭게 갱신.
		execSql($appSql);
    }
    ...
    if($app=='정렬' && $parent_idx!='') {
		$car_code = splitVB($parent_idx,'_-_')[0];
		$cate_code = splitVB($parent_idx,'_-_')[1];
        $addSql = "
CALL mis_normalize_sort_order_proc(
    'mis_parts_assy_define_detail', 
    'car_code', '$car_code', 
    'cate_code', '$cate_code', 
    'sort_num'
);
