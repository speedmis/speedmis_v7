...
function thisLogic_toolbar() {
    ...
    $("a#btn_1").text("아래 전체페이지의 인쇄대기열 삭제");
    $("li#btn_1_overflow").text("적용");
    $("#btn_1").css("background", "#88f");
    $("#btn_1").css("color", "#fff");
    $("#btn_1").click( function() {
        $("#grid").data("kendoGrid").dataSource.transport.options.read.data.app = "인쇄삭제";
...
function list_query() {
    ...
    if($app=='인쇄삭제') { 
		$appSql2 = " update g5_shop_item set print_request_time=null
		where " . replace(splitVB($countQuery, "where 9=9  and")[1], 'table_m.','');

		$appSql2 = $appSql2 . "; update mis_parts_storage set print_request_time=null
		where " . replace(splitVB($countQuery, "where 9=9  and")[1], 'table_m.','');

		execSql($appSql2);
		
		$resultMessage = '정상적으로 인쇄대기열에서 삭제되었습니다.';
		$resultCode = 'fail';
   }
...
function list_json_load() {
    ...
    global $data, $key_aliasName, $child_alias, $countQuery, $selectQuery, $keyword, $menuName;
	global $_count;  //총갯수
    ...
    if($flag=='readResult' && $addParam=='print') { 
        ...
        if($count>0) {
			...
			if($flag=='창고라벨인쇄') {
				...
				$full_name4 = splitVB($full_name, ' > ');
				$n1 = Trim($full_name4[1]) . Trim($full_name4[2]);
				$n2 = Trim($full_name4[3]) . Trim($full_name4[4]);
				$url = "https://globalautoparts.co.kr/gadmin/?gubun=6052&sid=$idx";
				//$g812name = $p_data[0]['vQng8name'].$p_data[0]['vQng12name'];
				//$g1620name = $p_data[0]['vQng16name'].$p_data[0]['vQng20name'];
$qr_format = "
^XSETCUT,DOUBLECUT,0
^Q40,3
^W80
...
Dy2-me-dd
Th:m:s
W373,49,5,2,M0,8,7,57,0
$url
ATB,35,58,82,82,0,0E,B,0,$n1
ATB,35,184,82,82,0,0E,B,0,$n2
E
...
";
				...
				$appSql2 = " update mis_parts_storage set print_request_time=null,print_response_time=NOW()
				where " . replace(splitVB($countQuery, "where 9=9  and")[1], 'table_m.','') . " and idx=$idx";
				...
				$full_name = $p_data[0]['full_name'];
				$full_name_n = splitVB($full_name, ' > ');
				$n1 = uni_left(Trim($full_name_n[0]) . ' ' . Trim($full_name_n[1]), 17);
				$n3 = Trim($full_name_n[2]);
				$n3a = Trim(splitVB($n3,'|')[0]);
				$n3b = Trim(splitVB($n3,'|')[1]);
				$n4 = Trim($full_name_n[3]);
				$n5 = Trim($full_name_n[4]);
				if(count($full_name_n)>5) {
					$n5 = $n5 . ' > ' . Trim($full_name_n[5]);
				}
				$url = "https://globalautoparts.co.kr/gadmin/?gubun=6052&pid=$idx";
				//$g812name = $p_data[0]['vQng8name'].$p_data[0]['vQng12name'];
				//$g1620name = $p_data[0]['vQng16name'].$p_data[0]['vQng20name'];
$qr_format = "
^XSETCUT,DOUBLECUT,0
...
Dy2-me-dd
Th:m:s
W416,39,5,2,M0,8,6,57,0
$url
ATB,32,30,45,45,0,0E,B,0,$n1
ATB,31,87,45,45,0,0E,B,0,$n3a
ATB,31,147,45,45,0,0E,B,0,$n3b
ATB,39,261,28,28,0,0E,B,0,$n5
ATB,32,203,45,45,0,0E,B,0,$n4
E


";
				$appSql2 = " update g5_shop_item set print_request_time=null,print_response_time=NOW()
				where " . replace(splitVB($countQuery, "where 9=9  and")[1], 'table_m.','') . " and it_id=$idx";
//echo $appSql2 ;
				execSql($appSql2);
